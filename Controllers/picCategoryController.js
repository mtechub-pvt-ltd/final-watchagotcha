...
export const create = async (req, res) => {
  try {
    const { name } = req.body;
    const createQuery =
      "INSERT INTO pic_category (name) VALUES ($1) RETURNING *";
...
export const deleteCategory = async (req, res) => {
  ...
  try {
    ...
    const oldCatgory = await getSingleRow("pic_category", condition);
    if (oldCatgory.length === 0) {
      return res
        .status(404)
        .json({ statusCode: 404, message: "Category not found " });
    }
    const delQuery = "DELETE FROM pic_category WHERE id=$1";
...
export const getSpecificCategory = async (req, res) => {
  try {
    ...
    const result = await getSingleRow("pic_category", condition);
    if (result.length === 0) {
      return res
        .status(404)
        .json({ statusCode: 404, message: "Category not found" });
...
export const getAllCategories = async (req, res) => {
  try {
    ...
    let picCategoryQuery = `SELECT * FROM pic_category ORDER BY created_at DESC`;

    if (req.query.page === undefined && req.query.limit === undefined) {
      console.log("00000");
      // If no query parameters for pagination are provided, fetch all categories without pagination
      picCategoryQuery = `SELECT * FROM pic_category ORDER BY created_at DESC`;
    } else {
      picCategoryQuery += ` LIMIT $1 OFFSET $2;`;
    }
    let queryParameters = [];
    ...
    const { rows } = await pool.query(picCategoryQuery, queryParameters);
    ...
    if (req.query.page === undefined && req.query.limit === undefined) {
      ...
      const totalCategoriesQuery = `SELECT COUNT(*) AS total FROM public.pic_category`;
...
export const updateCatgory = async (req, res) => {
  try {
    ...
    const updateCategory = `UPDATE pic_category SET name=$1, "updated_at"=NOW() WHERE id=$2 RETURNING *`;
...
export const deleteAllCategory = async (req, res) => {
  try {
    ...
    const query = "DELETE FROM pic_category RETURNING *";
