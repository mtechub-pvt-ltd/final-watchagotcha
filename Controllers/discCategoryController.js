...
export const create = async (req, res) => {
  try {
    const { name } = req.body;
    const createQuery =
      "INSERT INTO disc_category (name) VALUES ($1) RETURNING *";
...
export const deleteCategory = async (req, res) => {
  ...
  try {
    ...
    const oldCatgory = await getSingleRow("disc_category", condition);
    if (oldCatgory.length === 0) {
      return res
        .status(404)
        .json({ statusCode: 404, message: "Category not found " });
    }
    const delQuery = "DELETE FROM disc_category WHERE id=$1";
...
export const getSpecificCategory = async (req, res) => {
  try {
    ...
    const result = await getSingleRow("disc_category", condition);
    if (result.length === 0) {
      return res
        .status(404)
        .json({ statusCode: 404, message: "Category not found" });
...
export const getAllCategories = async (req, res) => {
  try {
    ...
    let discCategoryQuery = `SELECT * FROM disc_category ORDER BY created_at DESC`;

    if (req.query.page === undefined && req.query.limit === undefined) {
      console.log("00000");
      // If no query parameters for pagination are provided, fetch all categories without pagination
      discCategoryQuery = `SELECT * FROM disc_category ORDER BY created_at DESC`;
    } else {
    ...
    if (req.query.page === undefined && req.query.limit === undefined) {
      ...
      const totalCategoriesQuery = `SELECT COUNT(*) AS total FROM public.disc_category`;
...
export const updateCatgory = async (req, res) => {
  try {
    ...
    const query = "SELECT * FROM disc_category WHERE id=$1";
    const oldCatgory = await pool.query(query, [id]);
    if (oldCatgory.rows.length === 0) {
      return res.status(404).json({ message: "Category not found" });
    }

    const updateCategory = `UPDATE disc_category SET name=$1, "updated_at"=NOW() WHERE id=$2 RETURNING *`;
