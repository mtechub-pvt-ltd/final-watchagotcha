...
export const create = async (req, res) => {
  try {
    const { name } = req.body;
    const createQuery =
      "INSERT INTO video_category (name) VALUES ($1) RETURNING *";
...
export const deleteCategory = async (req, res) => {
  ...
  try {
    ...
    const oldCatgory = await getSingleRow("video_category", condition);
    if (oldCatgory.length === 0) {
      return res
        .status(404)
        .json({ statusCode: 404, message: "Video category not found " });
    }
    const delQuery = "DELETE FROM video_category WHERE id=$1";
    await pool.query(delQuery, [id]);
    res
      .status(200)
      .json({ statusCode: 200, message: "Video category deleted successfully",deleteCategory:oldCatgory[0] });
  } catch (error) {
    console.error(error);
    res.status(500).json({ statusCode: 500, message: "Internal server error" });
  }
...
export const getSpecificCategory = async (req, res) => {
  try {
    ...
    const result = await getSingleRow("video_category", condition);
    if(result.length===0){
        return res.status(404).json({ statusCode: 404, message:"Video category not found" });
      }
...
export const getAllCategories = async (req, res) => {
  try {
    ...
    let videoCategoryQuery = `SELECT * FROM video_category ORDER BY created_at DESC`;

    if (req.query.page === undefined && req.query.limit === undefined) {
      console.log("00000");
      // If no query parameters for pagination are provided, fetch all categories without pagination
      videoCategoryQuery = `SELECT * FROM video_category ORDER BY created_at DESC`;
    } else {
      videoCategoryQuery += ` LIMIT $1 OFFSET $2;`;
    }
    let queryParameters = [];
    ...
    const { rows } = await pool.query(videoCategoryQuery, queryParameters);
    ...
    if (req.query.page === undefined && req.query.limit === undefined) {
      ...
      const totalCategoriesQuery = `SELECT COUNT(*) AS total FROM public.video_category`;
...
export const updateCatgory = async (req, res) => {
  try {
    ...
    const query = "SELECT * FROM video_category WHERE id=$1";
    const oldCatgory = await pool.query(query, [id]);
    if (oldCatgory.rows.length === 0) {
      return res.status(404).json({ message: "Category not found" });
    }

    const updateCategory = `UPDATE video_category SET name=$1, "updated_at"=NOW() WHERE id=$2 RETURNING *`;
...
export const deleteAllCategory = async (req, res) => {
  try {
    ...
    const query = 'DELETE FROM video_category RETURNING *';
    const { rows } = await pool.query(query);

    if (rows.length === 0) {
      return res.status(404).json({
        statusCode: 404,
        message: 'No video category found to delete',
      });
    }

    res.status(200).json({
      statusCode: 200,
      message: 'All video category deleted successfully',
